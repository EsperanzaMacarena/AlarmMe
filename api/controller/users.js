...
let controller = {
  ...
  updateName: (req, res, next) => {
    const id = req.user._id;
    User.updateOne({ _id: id }, { $set: { fullname: req.body.fullname } }).exec(
      (error, response) => {
        if (error) res.send(404, error.message);
        else
  ...
  updatePassword: (req, res, next) => {
    ...
    if (req.body.password == req.body.passwordTwo) {
      ...
      User.updateOne({ _id: id }, { $set: { password: hash } }).exec(
        (error, response) => {
          if (error) res.send(404, error.message);
          else
            User.findById(id).exec((error, user) => {
      ...
      res.status(203).json({ result: "Password dont match" });
    }
  },
  disabledUser: (req, res, next) => {
    const id = req.params.id;
    User.updateOne({ _id: id }, { $set: { enabled: req.body.enabled } }).exec(
      (error, response) => {
        if (error) res.send(404, error.message);
        else
          User.findById(id).exec((error, user) => {
  ...
  deleteImg: (req, res, next) => {
    const id = req.user._id;
    User.updateOne({ _id: id }, { $set: { picture: undefined } }).exec(
      (error, response) => {
        if (error) res.send(404, error.message);
        else
          User.findById(id).exec((error, user) => {
